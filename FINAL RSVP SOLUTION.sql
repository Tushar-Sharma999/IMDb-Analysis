...
SELECT 
    'director_mapping' AS Table_Name, COUNT(*) AS Number_of_rows
FROM
    director_mapping 
UNION SELECT 
    'genre' AS TableName, COUNT(*) AS Number_of_rows
FROM
    genre 
UNION SELECT 
    'movie' AS TableName, COUNT(*) AS Number_of_rows
FROM
    movie 
UNION SELECT 
    'names' AS TableName, COUNT(*) AS Number_of_rows
FROM
    names 
UNION SELECT 
    'ratings' AS TableName, COUNT(*) AS Number_of_rows
FROM
    ratings 
UNION SELECT 
    'role_mapping' AS TableName, COUNT(*) AS Number_of_rows
FROM
    role_mapping;
...
SELECT
    COUNT(*) as Total_no_of_movies_with_one_genre
FROM
    one_genre_movies
WHERE
    Total_genre = 1;
...
max_votes AS
(
SELECT languages FROM language_wise_votes
ORDER BY votes DESC
LIMIT 1)
...
FROM
	...
	ON m.id = rm.movie_id
        ...
        USING(movie_id)
			...
			ON n.id = rm.name_id
WHERE category = 'Actor' AND m.country LIKE '%India%'
GROUP BY n.name
...
SELECT
    ...
    END AS movie_classified_as_per_rating
FROM
    movie AS m
INNER JOIN
...
-- Let us first check if there are any other currency other than $ in the 'worlwide_gross_income' column.
SELECT worlwide_gross_income 
FROM movie 
WHERE worlwide_gross_income NOT LIKE '%$%';

-- There are 3 entries whose currency is 'INR'. 
-- We are not taught how to convert values in a table using SQL code, so we ignore the currency and do calculation by considering the numbers only.
-- One possible solution to this issue is changing the values in database itself, but as we are told not o alter the database, we will not change them to $.
...
FROM
    ...
    movie AS m
    ON g.movie_id = m.id
ORDER BY year, worldwide_gross_income DESC
)
SELECT *
FROM incomewise_movies
WHERE movie_rank <=5;
